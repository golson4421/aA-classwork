...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    ...
    FROM routes a 
    RIGHT JOIN routes b ON (a.company = b.company AND a.num = b.num AND b.stop_id = 53)
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT a.num, a.company, transfer_stop, second_bus.num, second_bus.company
    FROM routes a
    JOIN stops stopa ON (a.stop_id = stopa.id)
    JOIN (SELECT DISTINCT stopb.name as transfer_stop, c.num, c.company
    FROM routes b
    JOIN routes c ON (b.company = c.company AND b.num = c.num)
    JOIN stops stopb ON (b.stop_id = stopb.id)
    JOIN stops stopc ON (c.stop_id = stopc.id)
    WHERE stopc.name = 'Sighthill') as second_bus ON stopa.name = second_bus.transfer_stop
    WHERE a.num in (SELECT num FROM routes WHERE stop_id = 53)
    AND a.company in (SELECT company FROM routes WHERE stop_id = 53)
  SQL
end
